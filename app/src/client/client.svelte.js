...
export const DATA = storage_var("data-v2", {
    initial: null,
    parse: JSON.parse,
...
export const refresh = async () => {
    ...
    if (!res.ok)
        throw new Error(`error fetching ${DATA_URL}: status code not OK: ${res.status} ${res.statusText}`);
    const data = await res.json();
    const stored = { obtained_at: new Date(), data };
    DATA.set(stored);
